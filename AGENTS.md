...
| Database | PostgreSQL (dev/test + prod, chosen 2026-07-09 per ArchiMate v2.4 Technology layer split — closes issue #11; SQLite may still suit local dev) |
...
## Requirements Model
- `EAxCRM-Requirements.md` holds 45 requirements with ID, Status, Version, GUID, parent hierarchy, and entity mappings
...
- **Resolved 2026-07-08 (issue #8)**: the 2026-07-02 plan below to use CRM-6 through CRM-9 (and SAL-5) for the Manage Customer Account *BPMN process*-level requirements was superseded 2026-07-07 by the Create Customer Account UI field/validation requirements (issue #7). The process-level requirements were finally written as **CRM-13 through CRM-16** (see "Manage Customer Account Process" below) — no SAL-5 was needed, since all four turned out to be Customer/Contact-domain. BPMN/ArchiMate linkage is expressed via Description/Rationale text referencing the specific activity/gateway and business process by name (the established convention — there's no automated connector from a Requirement to a BPMN/ArchiMate element in the generator).
- Original 2026-07-02 plan (**MD only, never generated into EA under these original IDs — see note above; content later became CRM-13..16**): create Customer Account from minimal data, fuzzy-match duplicate detection + merge, email history retrieval via IMAP scan, role-gated opt-in suggestion requiring user confirmation, and verify/create Customer Account when an RFQ arrives from an unrecognized org — all in support of the Manage Customer Account process
...
6. ~~Generate the Manage Customer Account process-level requirements~~ — **done 2026-07-08 (issue #8)**: CRM-13 (duplicate detection fuzzy-match), CRM-14 (merge semantics / `Customer.merged_into` audit trail), CRM-15 (email history retrieval trigger + re-scan dedup), CRM-16 (opt-in-suggest role-gated trigger, distinct from CRM-11's create-time default) — all children of CRM-1, generated live into `EAxCRM.qea` via `generate_requirements_from_md.py`. No fresh SAL-5 was needed; all four are Customer/Contact-domain, not sales-domain.
7. **Physical Data Model experiment (issue #16)** — run the phased plan at `docs/superpowers/plans/2026-07-09-physical-data-model-experiment-plan.md` against the sandbox slice; fill findings F1..F5 in the spec §7; post the adoption recommendation back to issue #16.
